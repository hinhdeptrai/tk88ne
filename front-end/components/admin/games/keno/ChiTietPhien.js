import CountdownTimer from "@/components/games/keno/CountdownTimer";
...
const ChiTietPhien = ({ ID, TYPE_GAME = "keno1p" }) => {
  ...
  const [ketQua, setKetQua] = useState(dataQuery?.ketQua?.length > 0 ? dataQuery.ketQua : [0, 0, 0, 0, 0]);
  const [ketQuaDieuChinh, setKetQuaDieuChinh] = useState([0, 0, 0, 0, 0]);
  ...
  useEffect(() => {
    if (dataQuery) {
      ...
      setKetQua(ketQua.length === 0 ? [0, 0, 0, 0, 0] : ketQua);
      ...
      >
        ...
        {dataQuery && (
          <>
            ...
            >
              Tình trạng: {convertJSXTinhTrangGameKeno(tinhTrang)}
            </Typography>
